...
namespace BadProject.Providers
{
    public class NoSqlAdvertisementAdapter : IAdvertisementProvider
    {
        ...
        public NoSqlAdvertisementAdapter(
            NoSqlAdvProvider provider,
            IOptions<AdvertisementServiceOptions> options,
            ILogger<NoSqlAdvertisementAdapter> logger)
        {
            _provider = provider ?? throw new ArgumentNullException(nameof(provider));
            if (options is null) throw new ArgumentNullException(nameof(options));
            _retryOptions = options.Value?.NoSqlProviderOptions ?? throw new ArgumentNullException(nameof(options) + "." + nameof(AdvertisementServiceOptions.NoSqlProviderOptions));
        ...
        public async Task<Advertisement?> GetAsync(string id)
        {
            ...
            do
            {
                try
                {
                    _logger.LogInformation(
                        "Attempting to fetch advertisement {Id} from NoSQL provider. Attempt {Attempt}/{MaxAttempts}",
                        id, retryCount + 1, _retryOptions.RetryCount + 1);

                    Advertisement? advertisement = _provider.GetAdv(id);

                    if (advertisement != null)
                    {
                        _logger.LogInformation("Successfully retrieved advertisement {Id} from NoSQL provider", id);
                        return advertisement;
                    }

                    _logger.LogWarning("No advertisement found with ID {Id}", id);
                    return null;
                }
                catch (Exception ex)
                {
                    ...
                    if (retryCount <= _retryOptions.RetryCount)
                    {
                        ...
                        _logger.LogWarning(ex,
                            "Error fetching advertisement {Id} from NoSQL provider. Attempt {Attempt}/{MaxAttempts}. Retrying in {Delay:N3}s",
            ...
            _logger.LogError(lastException,
                "Failed to fetch advertisement {Id} from NoSQL provider after {Attempts} attempts",
                id, retryCount);

            throw new AdvertisementProviderException(
                $"Failed to fetch advertisement {id} from NoSQL provider after {retryCount} attempts",
                lastException);
        }
    }

    public class AdvertisementProviderException : Exception
    {
        public AdvertisementProviderException(string message, Exception innerException)
